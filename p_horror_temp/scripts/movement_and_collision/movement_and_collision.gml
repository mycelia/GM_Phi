...
function movement_and_collision() {
	...
	//I don't remember where I originally got this code from. If you recognise it as yours, let me know...


	var _movement_direction=(round(argument[0])/90)*90;
	...
	if(_movement_direction !=-1 && !place_meeting(x+lengthdir_x(1,_movement_direction),y+lengthdir_y(1,_movement_direction),_wall))
	{   //We are not against a wall.  Move.
	    ...
	    {   //We hit a wall.  Back up until we aren't in a wall.
			...
			//if self = obj_mia_lib show_debug_message("AAAH A WALL");
	        _hit_wall=true;
	    }
	...
	{   //Check for ramps!  Now things are starting to get crazy.
	    ...
	    var _angle=45;
	    var _ramp=false;
	    if(!place_meeting(x+lengthdir_x(1,_movement_direction+_angle),y+lengthdir_y(1,_movement_direction+_angle),_wall))
	    {   //Found a ramp counter clockwise!
	        _movement_direction+=_angle;
	        _ramp=true;
	    }
	    else if(!place_meeting(x+lengthdir_x(1,_movement_direction-_angle),y+lengthdir_y(1,_movement_direction-_angle),_wall))
	    {   //Then clockwise
	        _movement_direction-=_angle;
	        _ramp=true;
	    }
	    ...
	    {   //Move along ramp;
	        ...
	        {   //We hit a wall.  Back up until we aren't in a wall.
	            x-=lengthdir_x(1,_movement_direction);
	            y-=lengthdir_y(1,_movement_direction);
				//if self = obj_mia_lib show_debug_message("AAA A RAMP");
	            _hit_wall=true;
	        }
	        if(point_distance(_temp_x,_temp_y,x,y)>_move_speed)
	        {
	        ...
	        {   //We broke out of the wall, so let's round stuff.
	            x=round(x);
	            y=round(y);
				//if self = obj_mia_lib show_debug_message("supposedly i am breaking out of the wall");
	        }
	    }
	    else
