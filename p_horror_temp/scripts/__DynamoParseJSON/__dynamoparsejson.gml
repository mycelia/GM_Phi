...
function __DynamoParseJSON(_string)
{
    ...
    var _parser = new __DynamoParseJSONParser(_buffer, buffer_get_size(_buffer), _cache_buffer);
...
function __DynamoParseJSONParser(_buffer, _buffer_size, _cache_buffer) constructor
{
    buffer          = _buffer;
                ...
                if (!was_string)
                {
                    switch(cache_value)
                    {
                        ...
                        default:
                            try
                            {
                                var _real_value = real(cache_value);
                                if (abs(_real_value) > (1 << 53))
                                {
                                    //We probably can't express this number accurately as a float, so let's try a 64-bit integer
                                    var _int64_value = int64(cache_value);
                                    if (string(_int64_value) == cache_value)
                                    {
                                        cache_value = _int64_value;
                                    }
                                    else
                                    {
                                        //Oh well, we tried
                                        cache_value = _real_value;
                                    }
                                }
                                else
                                {
            ...
            switch(value)
            {
                ...
                case ord("["):
                    ...
                    else if (!in_key)
                    {
                        ...
                        cache_started = true;
                        cache_value = (new __DynamoParseJSONParser(buffer, buffer_size, cache_buffer)).root;
                    }
                    else
                    {
                        show_error("\"[\" unexpected (position " + string(buffer_tell(buffer) - 1) + ")\n ", false);
