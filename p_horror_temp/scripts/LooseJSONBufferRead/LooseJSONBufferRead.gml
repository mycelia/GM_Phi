...
function LooseJSONBufferRead(_buffer, _inOffset = undefined)
{
    ...
    while(buffer_tell(_buffer) < _bufferSize)
    {
        ...
        {
            __BufferReadLooseJSONComment(_buffer, _bufferSize);
        }
        else if ((_byte == ord("/")) && (buffer_peek(_buffer, buffer_tell(_buffer), buffer_u8) == ord("*")))
        {
            __BufferReadLooseJSONMultilineComment(_buffer, _bufferSize);
        }
        else if (_byte == ord("["))
        {
            _result = __BufferReadLooseJSONArray(_buffer, _bufferSize);
        }
        else if (_byte == ord("{"))
        {
            _result = __BufferReadLooseJSONStruct(_buffer, _bufferSize);
        }
        else if (_byte > 0x20)
        {
...
function __BufferReadLooseJSONArray(_buffer, _bufferSize)
{
    var _result = [];
    ...
    while(buffer_tell(_buffer) < _bufferSize)
    {
        ...
        {
            var _value = __BufferReadLooseJSONValue(_buffer, _bufferSize, _byte);
...
function __BufferReadLooseJSONStruct(_buffer, _bufferSize)
{
    var _result = {};
    ...
    while(buffer_tell(_buffer) < _bufferSize)
    {
        ...
        {
            var _key = __BufferReadLooseJSONValue(_buffer, _bufferSize, _byte);
            ...
            else //Is an array
            {
                ...
                repeat(_keyArrayLength-1)
                {
                    ...
                    _result[$ _key] = __BufferReadLooseJSONDeepCopyInner(_value, self, self);
                    ++_i;
                }
            }
...
function __BufferReadLooseJSONValue(_buffer, _bufferSize, _firstByte)
{
    if (_firstByte == ord("["))
    {
        return __BufferReadLooseJSONArray(_buffer, _bufferSize);
    }
    else if (_firstByte == ord("{"))
    {
        return __BufferReadLooseJSONStruct(_buffer, _bufferSize);
    }
    else if (_firstByte == ord("\""))
    {
        return __BufferReadLooseJSONDelimitedString(_buffer, _bufferSize);
    }
    else if ((_firstByte == ord("@")) && (buffer_peek(_buffer, buffer_tell(_buffer), buffer_u8) == ord("'")))
    {
        buffer_seek(_buffer, buffer_seek_relative, 1);
        return __BufferReadLooseJSONMultilineString(_buffer, _bufferSize);
    }
    else
    {
        return __BufferReadLooseJSONString(_buffer, _bufferSize);
    }
}

function __BufferReadLooseJSONDelimitedString(_buffer, _bufferSize)
{
...
function __BufferReadLooseJSONMultilineString(_buffer, _bufferSize)
{
    static _cacheBuffer = buffer_create(1024, buffer_grow, 1);
    buffer_seek(_cacheBuffer, buffer_seek_start, 0);
...
function __BufferReadLooseJSONString(_buffer, _bufferSize)
{
    static _cacheBuffer = buffer_create(1024, buffer_grow, 1);
    buffer_seek(_cacheBuffer, buffer_seek_start, 0);
...
function __BufferReadLooseJSONComment(_buffer, _bufferSize)
{
    while(buffer_tell(_buffer) < _bufferSize)
    {
        var _byte = buffer_read(_buffer, buffer_u8);
...
function __BufferReadLooseJSONMultilineComment(_buffer, _bufferSize)
{
    while(buffer_tell(_buffer) < _bufferSize)
    {
        var _byte = buffer_read(_buffer, buffer_u8);
...
function __BufferReadLooseJSONDeepCopyInner(_value, _oldStruct, _newStruct)
{
    var _copy = _value;
    ...
    else if (is_struct(_value))
    {
        ...
        repeat(array_length(_namesArray))
        {
            ...
            _copy[$ _name] = __BufferReadLooseJSONDeepCopyInner(_value[$ _name], _value, _copy);
            ++_i;
        }
    }
    else if (is_array(_value))
    {
        ...
        repeat(_count)
        {
            _copy[@ _i] = __BufferReadLooseJSONDeepCopyInner(_value[_i], _oldStruct, _newStruct);
            ++_i;
        }
    }
